	...
	, ':: FunctionDeclaration,FunctionExpression,ArrowFunctionExpression': function functionDestructuringAndDefaultsAndRest(node, astQuery) {
		{
			...
			var isArrowFunction = node.type === "ArrowFunctionExpression";
			var isNakedFunction = node.expression === true;//isArrowFunction && functionBody.type !== "BlockStatement";
			var fnBodyIsNotABlockStatement = isNakedFunction && functionBody.type !== "BlockStatement";
			var fnBodyIsSequenceExpression = fnBodyIsNotABlockStatement && functionBody.type === "SequenceExpression";
			var fnBodyHasHiddenBrackets =
			...
			var initialParamsCount = paramsCount;
			var fnBodyStart = core.__getNodeBegin(node)
				, fnBodyEnd = functionBody.range[1]
			;
			var defaultsCount = defaults.length;
			var lastParam = params[paramsCount - 1];
			var lastDflt = defaults[defaultsCount - 1];

			var fnHasNoParams = paramsCount === 0 && !rest;

			var insertIntoBodyBegin = "", insertIntoBodyEnd = "";
			...
			if( isNakedFunction || isArrowFunction ) {
				if( fnBodyHasHiddenBrackets || isArrowFunction || fnBodyIsSequenceExpression ) {
					...
					var lastDefinition = rest || (lastDflt || lastParam);
					...
					var str = this.alter.get(left, fnBodyStart);

					var arrowFunctionParamsHasNoBrackets =// let arrow = a=>a
						isArrowFunction
						&& !fnHasNoParams
						&& !rest
			...
			if( paramsCount ) {
				for(var i = 0 ; i < paramsCount ; i++) {
					var param = params[i];
					var prevParam = params[i - 1];
					var doesArgumentsInsideFunction = node.$scope.doesArgumentsUsing();
					var _isObjectPattern = isObjectPattern(param);
					var _isArrayPattern = !_isObjectPattern && isArrayPattern(param);

					if( _isObjectPattern || _isArrayPattern ) {

						var elements = (_isObjectPattern ? param.properties : param.elements)
							, elementsLength = elements.length
							, element = elements[elementsLength - 1]
						;
						var functionParamReplacer =
						...
						var paramReplacerName = (_isObjectPattern || _isArrayPattern) && !doesArgumentsInsideFunction
			...
			if( defaultsCount ) {
				for(var i$0 = 0 ; i$0 < defaultsCount ; i$0++) {
					var paramIndex = initialParamsCount - defaultsCount + i$0;
					var param$0 = params[paramIndex];
					var prevDflt = defaults[i$0 - 1];
					var prevParam$0 = params[paramIndex - 1];
					var dflt = defaults[i$0];

					if (dflt.type === "Identifier" && dflt.name === param$0.name) {
						error(getline(node), "function parameter '{0}' defined with default value refered to scope variable with the same name '{0}'", param$0.name);
					}

					var defaultStr = void 0;
					if( isObjectPattern(param$0) || isArrayPattern(param$0) ) {
						defaultStr =
							destructuring.unwrapDestructuring(
								"var"
								, param$0
						...
						defaultStr = "var "
							+ core.definitionWithDefaultString(param$0, "arguments[" + paramIndex + "]", this.alter.get(dflt.range[0], dflt.range[1]))
							+ ";"
					}

					param$0.$replaced = true;
...
//					this.alter.setState('default_remove');
					this.alter.remove(
						((prevDflt || prevParam$0) ? ((prevDflt || prevParam$0).range[1] + 1) : param$0.range[0]) - (prevParam$0 ? 1 : 0)
						, dflt.range[1]
					);
...
//				this.alter.restoreState();
	...
	, replaceThisInArrowFunction: function(node, astQuery) {
		...
		var self = this;
		var thisUniqueName = self._thisUniqueName;
